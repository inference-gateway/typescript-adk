...
const SERVER_URL = process.env['SERVER_URL'] ?? 'https://127.0.0.1:8443';
const PROMPT =
  process.env['PROMPT'] ?? 'Hello over TLS - please echo this back to me.';
const POLL_INTERVAL_MS = 500;
const POLL_MAX_ATTEMPTS = 20;

const tls: ClientTLSConfig = loadClientTLSConfigFromEnv() ?? {
  caPath: process.env['CLIENT_TLS_CA_PATH'] ?? './certs/cert.pem',
};

const client = createA2AClient({ baseURL: SERVER_URL, tls });
