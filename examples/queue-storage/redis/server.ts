...
const AGENT_NAME = process.env['A2A_AGENT_NAME'] ?? 'queue-storage-redis-agent';
const AGENT_DESCRIPTION =
  process.env['A2A_AGENT_DESCRIPTION'] ??
  'A2A echo agent backed by RedisTaskStorage. The queue and dead-letter store survive restarts.';
...
const KEY_PREFIX = process.env['REDIS_KEY_PREFIX'] ?? 'a2a:';
...
// The only block that differs between the in-memory and redis variants:
// connect to Redis, hydrate the in-memory mirror, and disconnect on shutdown.
// `redisConnectOptionsFromEnv()` reads REDIS_URL, falling back to
// REDIS_HOST / REDIS_PORT / REDIS_PASSWORD / REDIS_DB.
const fromEnv = redisConnectOptionsFromEnv();
const storage = await RedisTaskStorage.connect({
  ...fromEnv,
  ...(fromEnv.url === undefined && fromEnv.options === undefined
...
console.log(`  storage:    redis (prefix=${KEY_PREFIX})`);
